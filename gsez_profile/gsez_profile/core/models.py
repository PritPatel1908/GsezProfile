...
def generate_gsezid():
    ...
    # Find the highest sequential number used so far
    last_user = User.objects.all().order_by('-gsezid').first()
    if last_user and last_user.gsezid and last_user.gsezid.startswith('ZIS'):
        try:
            # Extract the sequence number from the end of the GSEZ ID
            sequence_number = int(last_user.gsezid[-6:])
            # Increment by 1
        ...
        # If no users with ZIS format exist, start from 1
        new_sequence = 1

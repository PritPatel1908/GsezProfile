...
class UserRegistrationForm(UserCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Make these fields required for regular users but optional for admin creation
    ...
    def clean(self):
        ...
        if user_type == 'user':
            if not cleaned_data.get('nationality'):
                self.add_error('nationality', 'This field is required for regular users.')
            if not cleaned_data.get('date_of_birth'):
                self.add_error('date_of_birth', 'This field is required for regular users.')
            if not cleaned_data.get('current_address'):
                self.add_error('current_address', 'This field is required for regular users.')
